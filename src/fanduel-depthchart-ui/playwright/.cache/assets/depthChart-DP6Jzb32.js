...
const baseURL = "http://api.test/api/sports/nfl/teams/TB/depthchart";
const http = axios.create({ baseURL });
const addPlayer = (req) => http.post("/", req);
const removePlayer = (position, number) => http.delete(`/${position}/${number}`).then((r) => r.data);
const getBackups = (position, number) => http.get(`/${position}/${number}/backups`).then((r) => r.data);
const getFullChart = (opts) => http.get("/", { signal: opts?.signal }).then((r) => r.data);

export { addPlayer as a, getBackups as g, removePlayer as r, toApiError as t };
//# sourceMappingURL=depthChart-DP6Jzb32.js.map
